#1) Escribir una función que reciba una cadena de caracteres. La función deberá evaluar si la cadena recibida representa un número binario, y en ese caso devolver True, de lo contrario, deberá devolver False.
...
# 2) Escribir una función que reciba una cadena de caracteres a validar, y un segundo parámetro, que contenga una cadena con los caracteres válidos. La función debe devolver True, si la cadena a validar, está formada sólo por caracteres válidos; en caso contrario, deberá devolver False.
...
def formatearTexto(texto):
    textoFormateado = ""
    oraciones = texto.split(".") # Por cada punto separamos las oraciones
    for i in range(len(oraciones)): # Por cada oración (en vez de i podra haber usado oracion)
        oracion = oraciones[i].strip() # Quitamos los espacios en blanco
        if i == 0: # Si es la primera oración
            textoFormateado += oracion.capitalize() # Agregamos la oración formateada
        else:
            textoFormateado += ". " + oracion.capitalize() #  Agregamos la oración formateada con un punto y espacio
    return textoFormateado

def main():
    texto = input("Ingrese un texto en mayúsculas: ")
    textoFormateado = formatearTexto(texto)
