...
3. Utilizamos una Cola de Prioridad (Min-Heap) para llevar el registro de las salas activas. El Heap ordenará las salas basándose en en qué momento se liberan.
...
- Iterar sobre la lista ordenada toma N pasos. En cada paso, podemos hacer operaciones de Heap (heappop y heappush). El tamaño máximo del Heap es N (si todas las charlas se superponen). Las operaciones de Heap cuestan O(log N). Por lo tanto, el ciclo toma O(N log N).
...
Demostraremos que el algoritmo utiliza la cantidad mínima posible de salas utilizando 
el argumento de la "profundidad" de los intervalos.

Definimos la "profundidad" (d) de un conjunto de intervalos como la cantidad máxima de 
intervalos que se superponen en un mismo instante de tiempo. Es evidente que, como regla 
general, cualquier planificación válida necesitará AL MENOS 'd' salas para evitar 
superposiciones.

Supongamos que nuestro algoritmo Greedy termina abriendo 'k' salas. 
El algoritmo únicamente decide abrir la sala número 'k' cuando está procesando una 
charla 'C' y descubre que no puede ubicarla en ninguna de las 'k-1' salas ya abiertas.
Esto significa que en el instante de inicio de la charla 'C', las 'k-1' charlas que 
están actualmente en las otras salas comenzaron en el mismo instante o ANTES (porque 
ordenamos por inicio), y todavía NO han terminado.
Por lo tanto, en el instante exacto en que comienza la charla 'C', existen exactamente 
'k' charlas superponiéndose simultáneamente (las 'k-1' previas + la charla 'C').

Esto demuestra que la profundidad del conjunto de charlas en ese instante es 'k'. 
Si la profundidad es 'k', matemáticamente es imposible resolver el problema con menos 
de 'k' salas. Como nuestro algoritmo Greedy utiliza exactamente 'k' salas, queda 
demostrado que encuentra la solución óptima.
"""
